...
namespace OpenLoco::Paint
{
    ...
    static void paintTrainStationStyle0DiagonalTrack(PaintSession& session, const World::TileElementEntry& entry, const ImageId imageBase, const ImageId imageTranslucentBase)
    {
        ...
        // This was part of paintStationCargo
        const auto unkPosHash = (session.getUnkPosition().x + session.getUnkPosition().y) / 32;
        const auto cargoOffsets = stationObj->getCargoDiagonalOffsets(rotation, unkPosHash & 0x3);

        // Same segment flag table as paintTrainStationStyle0StraightTrack
        constexpr std::array<std::array<uint8_t, 4>, 4> cargoRotationFlags = {
            std::array<uint8_t, 4>{ 0x9, 0x5, 0x6, 0xA },
            std::array<uint8_t, 4>{ 0x9, 0x5, 0x6, 0xA },
            std::array<uint8_t, 4>{ 0xA, 0x6, 0x5, 0x9 },
        ...
        else
        {
            switch (seqIndex)
            {
                ...
                case 1:
                {
                    ...
                    // Canopy
                    World::Pos3 canopyBbOffset = World::Pos3{ 6, 6, 26 } + heightOffset;
                    World::Pos3 canopyBbSize = World::Pos3{ 2, 2, 1 };
                    session.addToPlotList4FD150(imageBase.withIndexOffset(TrainStation::ImageIds::Style0::diagonalCanopyNE1), heightOffset, canopyBbOffset, canopyBbSize);
                    session.attachToPrevious(imageTranslucentBase.withIndexOffset(TrainStation::ImageIds::Style0::diagonalCanopyTranslucentNE1), { 0, 0 });
                    break;
                }
                case 2:
    ...
    void paintTrainStation(PaintSession& session, const World::TileElementEntry& entry)
    {
        ...
        if (elStation.isGhost() || elStation.isAiAllocated())
        {
            ...
            imageIdbase = Gfx::applyGhostToImage(stationObj->imageOffsets[elStation.sequenceIndex()]);
            imageIdTranslucentBase = ImageId{ stationObj->imageOffsets[elStation.sequenceIndex()] }.withTranslucency(ExtColour::unk2F);

            // TODO: apply company colour if playerCompanyID != elTrack.owner()?
        }
        else
        {
            imageIdbase = ImageId{ stationObj->imageOffsets[elStation.sequenceIndex()], companyColour };
            imageIdTranslucentBase = ImageId{ stationObj->imageOffsets[elStation.sequenceIndex()] }.withTranslucency(translucentColour);
        }
